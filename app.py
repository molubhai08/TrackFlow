...
- Only query database when necessary
    ...
    chart_data = None
    if any(keyword in user_query.lower() for keyword in ['visualize', 'visualise', 'plot', 'chart', 'graph', 'line graph']):
        ...
        # Use Groq function calling to extract structured data
        try:
            ...
            function_schema = {
                ...
                "description": "Extract x-axis (features) and y-axis (numeric values) from agent results or database query outputs.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "x_axis": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "List of feature values to plot on the x-axis (e.g., dates, categories)."
                        },
                        "y_axis": {
                            "type": "array",
                            "items": {"type": "number"},
                            "description": "List of numeric values corresponding to each x-axis item."
                        },
                        "message": {
                            "type": "string",
                            "description": "Plain explanation of what the agent did."
                        }
                    },
...

{
  ...
  "message": string     # e.g. "Retrieved water intake logs."
}

Rules:
- If the agent output is a query result: fill x_axis and y_axis with aligned values
...
"""
            ...
            print(f"[API Call #{memory.api_call_count + 1}] Extracting visualization data...")
            memory.increment_api_call()
            ...
            if x_axis and y_axis and len(x_axis) == len(y_axis):
                # Create line graph
                plt.figure(figsize=(10, 6))
                plt.plot(x_axis, y_axis, marker='o', linewidth=2.5, markersize=8, 
                        color='#667eea', markerfacecolor='#764ba2')
                
                # Determine labels from query
                xlabel = 'Date'
                ylabel = 'Value'
                title = 'Data Visualization'
                
                if 'water' in user_query.lower():
                    ylabel = 'Water Intake (units)'
                    title = 'Daily Water Intake Tracker'
                elif 'nutrition' in user_query.lower() or 'food' in user_query.lower():
                    ylabel = 'Amount'
                ...
                print(f"\n✅ Line graph created with {len(x_axis)} data points")
                print(f"Data extracted: {arguments}")
